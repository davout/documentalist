module Documentalist
  module OpenOffice
    ...
    def self.convert(origin, options)
      job = Delayed::Job.enqueue Converter.new(origin, options)
      debugger
      while job.reload.attempts == 0
        sleep(1)
      end
      options[:to]
    end
    ...
    module Server
      ...
      class ServerNotAvailable < Error; end
    end

  end

  class Converter < Struct.new(:origin, :options)
    # Converts documents
    def perform(origin, options)
      ...
      OpenOffice::Server.timeout(Documentalist.config[:open_office][:max_conversion_time], :attempts => Documentalist.config[:open_office][:max_conversion_attempts]) do
